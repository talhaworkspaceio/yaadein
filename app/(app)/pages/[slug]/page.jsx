...
export default function CustomPage({ params }) {
  ...
  useEffect(() => {
    ...
    async function fetchCustomPage() {
      if (!slug) return;
      try {
        const res = await fetch(`/api/pages?where[slug][equals]=${slug}&depth=2`, {
          cache: "no-store",
          headers: {
  ...
  if (notFound || !pageData) {
    return (
      <div style={{ minHeight: "100vh", background: "#050403", color: "#E0D7CD", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "20px" }}>
        <Navbar />
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: "42px", color: "var(--accent)" }}>404 - Page Not Found</h1>
        <p style={{ fontFamily: "var(--font-serif)", color: "var(--text2)", fontSize: "16px" }}>The page you are looking for does not exist or has not been published yet.</p>
    ...
    <div style={{ minHeight: "100vh", background: "#050403", color: "#E0D7CD", display: "flex", flexDirection: "column" }}>
      ...
      <main style={{ flex: 1, padding: "120px 20px 80px", maxWidth: "1200px", width: "100%", margin: "0 auto" }}>
        <style dangerouslySetInnerHTML={{ __html: `
          ...
          .pb-hero { text-align: center; padding: 60px 20px; background: rgba(28,15,7,0.4); border-radius: 16px; border: 1px solid rgba(184,134,11,0.2); }
