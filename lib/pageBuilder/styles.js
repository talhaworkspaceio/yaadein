...
export function blockDeclarations(block, device = "desktop", onlyOverrides = false) {
  ...
  // Full bleed — emitted after width/max-width and the margins above so it wins.
  // Pulls the block out of whatever centred, padded container it sits in, so a
  // background image reaches both edges of the screen. html/body already clip
  // horizontal overflow, so 100vw is safe alongside a vertical scrollbar.
  if (has("fullBleed") && dv("fullBleed")) {
    push(out, "width", "100vw");
    push(out, "max-width", "100vw");
    push(out, "margin-left", "calc(50% - 50vw)");
    push(out, "margin-right", "calc(50% - 50vw)");
  }
