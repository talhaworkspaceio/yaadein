...
export const SHADOW_PRESETS = [
  ...
  { name: "Gold Ambient Glow", val: "0 0 20px rgba(181, 139, 92, 0.35)" },
...
export const GRADIENT_PRESETS = [
  ...
  { name: "Warm Brass Glow", val: "radial-gradient(ellipse 70% 50% at 50% 0%, rgba(181,139,92,0.25) 0%, #080605 70%)" },
  { name: "Deep Walnut", val: "linear-gradient(160deg, #241a10 0%, #0b0806 60%, #050403 100%)" },
  { name: "Gold Sheen", val: "linear-gradient(135deg, #B58B5C 0%, #CBA378 50%, #B58B5C 100%)" },
  { name: "Charcoal Fade", val: "linear-gradient(180deg, #1a1a1a 0%, #050403 100%)" },
  { name: "Ember", val: "linear-gradient(135deg, rgba(181,139,92,0.2) 0%, rgba(20,12,6,0.9) 100%)" },
];
...
export const LAYOUT_FIELDS = [
  ...
  f("fullBleed", "Full bleed (edge to edge)", "toggle", {
    ...R,
    hint: "Breaks the block out of the page's centred column so its background runs to both edges of the screen.",
  }),
...
export const EFFECT_FIELDS = [
  ...
  f("hoverBg", "Hover Background", "text", { placeholder: "rgba(181,139,92,.15)" }),
...
export const BASE_DEFAULTS = {
  ...
  bgRepeat: "no-repeat",
  borderStyle: "none",
  borderColor: "rgba(181,139,92,0.3)",
  shadowPreset: "none",
  opacity: 1,
  paddingTop: 10,
  paddingRight: 10,
...
export const COMPONENTS = [
  ...
  },
  {
    ...
    defaults: withBase({
      ...
      borderBottomWidth: 1,
      borderLeftWidth: 1,
      borderColor: "rgba(181,139,92,0.25)",
      radiusTopLeft: 14,
      radiusTopRight: 14,
  ...
  },
  {
    ...
    defaults: withBase({
      ...
      borderBottomWidth: 1,
      borderLeftWidth: 1,
      borderColor: "rgba(181, 139, 92, 0.2)",
      radiusTopLeft: 12,
      radiusTopRight: 12,
  ...
  },
  {
    ...
    defaults: withBase({
      ...
      textColor: "#FFD700",
      bgType: "gradient",
      bgGradient: "linear-gradient(135deg, rgba(181,139,92,0.2) 0%, rgba(20,12,6,0.9) 100%)",
      borderStyle: "solid",
      borderTopWidth: 1,
      borderRightWidth: 1,
      borderBottomWidth: 1,
  ...
  // ------------------------------------------------------- coded sections
  {
    ...
    icon: "⬓",
    description: "A band of this page that is built in React. Its innards stay in code so the cart, players and customizer keep working — but you can move it, hide it, space it and style the wrapper like any other block.",
    defaults: withBase({
      type: "coded-section",
      sectionId: "",
      paddingTop: 0,
  ...
  // ------------------------------------------------- live site data blocks
  {
    id: "product-grid",
    name: "Frame / Product Grid",
    category: "Live Site Data",
    icon: "▦",
    description: "Pulls real frames from your Frame Catalog. Filter and cap them here; the grid stays current as the catalogue changes.",
    defaults: withBase({
      type: "product-grid",
      category: "all",
      orientation: "all",
      ...
      showCategory: true,
      showButton: false,
      buttonText: "View frame",
      excludeBoardGames: true,
      accentColor: "#B58B5C",
      tablet: { columns: "2" },
      mobile: { columns: "1" },
    }),
    contentFields: [
      f("category", "Category", "select", {
        options: [
          ...
          { label: "Nikkah Nama Frame", value: "Nikkah Nama Frame" },
          { label: "Service", value: "Service" },
          { label: "Board Game", value: "Board Game" },
        ],
      }),
      f("orientation", "Orientation", "select", {
        options: [
          { label: "Any", value: "all" },
          { label: "Portrait", value: "portrait" },
          { label: "Landscape", value: "landscape" },
      ...
      f("sort", "Sort by", "select", {
        options: [
          { label: "Catalogue order", value: "default" },
          { label: "Price: low to high", value: "price-asc" },
          { label: "Price: high to low", value: "price-desc" },
          { label: "Name A–Z", value: "name" },
        ],
      }),
      f("limit", "How many to show", "number", { min: 1, max: 48 }),
      f("columns", "Columns", "select", { ...R, options: ["1", "2", "3", "4", "5", "6"].map((n) => ({ label: `${n} across`, value: n })) }),
      f("imageRatio", "Image Ratio", "select", {
        options: [
          { label: "Portrait 3:4", value: "3 / 4" },
          { label: "Square", value: "1 / 1" },
      ...
      f("showPrice", "Show price", "toggle"),
      f("showCategory", "Show category label", "toggle"),
      f("showButton", "Show button", "toggle"),
      f("buttonText", "Button label", "text", { when: (b) => !!b.showButton }),
      f("excludeBoardGames", "Hide board games", "toggle"),
      f("accentColor", "Accent colour", "color"),
    ],
  },
  {
    ...
    category: "Live Site Data",
    icon: "◫",
    description: "Pulls your published services straight from Services Content, each linking to its own service page.",
    defaults: withBase({
      type: "services-grid",
      limit: 4,
      columns: "2",
    ...
    contentFields: [
      f("limit", "How many to show", "number", { min: 1, max: 24 }),
      f("columns", "Columns", "select", { ...R, options: ["1", "2", "3", "4"].map((n) => ({ label: `${n} across`, value: n })) }),
      f("showImage", "Show image", "toggle"),
      f("imageRatio", "Image Ratio", "select", {
        when: (b) => b.showImage !== false,
        options: [
      ...
      f("showDescription", "Show description", "toggle"),
      f("descriptionLines", "Description lines", "number", { min: 1, max: 8, when: (b) => b.showDescription !== false }),
      f("showPrice", "Show starting price", "toggle"),
      f("showButton", "Show button", "toggle"),
      f("buttonText", "Button label", "text", { when: (b) => b.showButton !== false }),
      f("accentColor", "Accent colour", "color"),
    ],
  },
  {
    ...
    category: "Live Site Data",
    icon: "⬚",
    description: "Links to each catalogue category, read live from your Categories list.",
    defaults: withBase({
      type: "category-tiles",
      limit: 8,
      columns: "4",
    ...
    contentFields: [
      f("limit", "How many to show", "number", { min: 1, max: 24 }),
      f("columns", "Columns", "select", { ...R, options: ["2", "3", "4", "5", "6"].map((n) => ({ label: `${n} across`, value: n })) }),
      f("accentColor", "Accent colour", "color"),
    ],
  },
  ...
  {
    id: "ambient-blobs",
    name: "Ambient Light Blobs",
    category: "Yaadein Studio",
    icon: "🌌",
    // Fills the section it is dropped into and sits behind the content, so it
    // needs no size of its own — hence position absolute in the defaults.
    defaults: withBase({
      type: "ambient-blobs",
      blobColorA: "#B58B5C",
      blobColorB: "#8B5E3C",
      ...
      paddingLeft: 0,
      marginBottom: 0,
      mobile: { blobSizeA: 300, blobSizeB: 320 },
    }),
    contentFields: [
      f("blobColorA", "Blob A Colour", "color"),
      f("blobColorB", "Blob B Colour", "color"),
      f("showGlow", "Show drifting glow", "toggle"),
      f("glowColor", "Glow Colour", "color", { when: (b) => b.showGlow !== false }),
      f("blobIntensity", "Intensity", "range", { min: 0, max: 1, step: 0.02 }),
      f("blobBlur", "Softness (blur px)", "number", { min: 0, max: 300 }),
      f("stretchToScreen", "Stretch across screen", "toggle", {
        hint: "Spans the full viewport width instead of the page's centred column, so the glow has no visible edge.",
      }),
      f("showPane", "Frosted glass overlay", "toggle", {
        hint: "The sheet the site's own sections lay over these blobs. Turn it off for raw, stronger colour.",
      }),
      f("paneBlur", "Frost Blur (px)", "number", { min: 0, max: 120, when: (b) => b.showPane !== false }),
      f("paneTint", "Frost Tint", "color", { when: (b) => b.showPane !== false }),
      f("paneOpacity", "Frost Strength", "range", { min: 0, max: 1, step: 0.02, when: (b) => b.showPane !== false }),
      f("blobSpeed", "Drift Speed (seconds per cycle)", "number", { ...R, min: 4, max: 120 }),
      f("blobSizeA", "Blob A Size (px)", "number", { ...R, min: 80, max: 1600 }),
      f("blobSizeB", "Blob B Size (px)", "number", { ...R, min: 80, max: 1600 }),
    ],
  },
  {
    id: "studio-lamp",
