...
export async function sendQueryNotification(queryData, customConfig = {}) {
  let serverUrl = customConfig.serverUrl || process.env.NTFY_SERVER_URL || 'https://ntfy.sh';
  const topic = customConfig.topic || process.env.NTFY_TOPIC || 'yaadein-orders';
  const authToken = customConfig.authToken || process.env.NTFY_AUTH_TOKEN;
  ...
  const reference = queryData.reference || 'N/A';
  const customerName = queryData.customer?.name || 'Customer';
  const phone = queryData.customer?.phone || 'N/A';
  const email = queryData.customer?.email || '';
  const service = queryData.service?.title || 'Custom framing';

  const d = queryData.dimensions || {};
  const size = [d.width, d.height].filter(Boolean).length
    ? `${d.width || '?'} x ${d.height || '?'} ${d.unit || ''}`.trim()
    : 'Not specified';
  ...
  if (email) messageLines.push(`✉️ Email: ${email}`);
  messageLines.push(`🖼️ Service: ${service}`);
  messageLines.push(`📐 Size: ${size}`);
  if (queryData.frame?.name) messageLines.push(`🪵 Frame: ${queryData.frame.name}`);
  if (typeof queryData.startingPrice === 'number') {
    messageLines.push(`💰 Starting price: Rs. ${queryData.startingPrice.toLocaleString()}`);
  }
  if (queryData.notes) messageLines.push(`\n📝 Notes:\n${queryData.notes}`);

  const payload = {
    topic: topic,
    title: `📐 New Quote Request: ${reference}`,
    message: messageLines.join('\n'),
    priority: 4,
    tags: ['straight_ruler', 'speech_balloon', 'frame_photo'],
  };

  // Quotes are settled on the phone, so make calling back one tap away.
  if (phone && phone !== 'N/A') {
    const cleanPhone = phone.replace(/[^\d+]/g, '');
    if (cleanPhone) {
      payload.actions = [
  ...
  try {
    ...
    const resData = await response.json().catch(() => ({}));
    console.log('[ntfy] Quote request notification sent successfully:', resData);
    return { success: true, data: resData };
  } catch (err) {
    console.error('[ntfy] Error dispatching notification:', err);
    return { success: false, error: err.message };
